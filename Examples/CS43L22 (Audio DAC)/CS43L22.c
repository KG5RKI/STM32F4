...
void CS43L22_I2C1_Write(uint8_t device_addr, uint8_t MAP_byte, uint8_t data)
{
	...
	while (!((I2C1->SR1 >> 7) & 0x1));			//Send MAP byte, auto-increment off
	I2C1->DR = MAP_byte;
	/* Check ACK */
	
	while (!((I2C1->SR1 >> 7) & 0x1));			//Send data to be written
	I2C1->DR = data;
	/* Check ACK */
	I2C1_GenerateStop();
}
...
void CS43L22_PowerUp()
{
	...
	/* Start MCLK with appropriate frequency */
	
	/* Set POWER_CONTROL_1 register to 0x9E */
